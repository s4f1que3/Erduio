...
export default function TeacherProfile() {
  ...
  const { data: myProfile } = useQuery({
    ...
    queryFn: async () => (await api.get("/teacher/me/profile")).data as { id: string; pfp_path?: string | null },
  });
  const { data: avatarUrl } = useQuery({
    queryKey: ["teacher-me-pfp", myProfile?.id],
    queryFn: async () => (await api.get(`/teacher/profile-pic/${myProfile?.id}`)).data as string,
  ...
  const uploadPic = useMutation({
    mutationFn: (file: File) => {
      ...
      return api.post("/teacher/profile-pic/add", form, { headers: { "Content-Type": "multipart/form-data" } });
    },
    onSuccess: () => { toast.success("Profile picture updated"); qc.invalidateQueries({ queryKey: ["teacher-me-profile"] }); },
    onError: (e) => toast.error(getErrorMessage(e)),
  });
  const deletePic = useMutation({
    mutationFn: () => api.post("/teacher/profile-pic/delete"),
    onSuccess: () => { toast.success("Profile picture removed"); qc.invalidateQueries({ queryKey: ["teacher-me-profile"] }); },
    onError: (e) => toast.error(getErrorMessage(e)),
  });
  ...
  async function saveInfo(d: { name: string; phone: string }) { setLi(true); try { await api.patch("/teacher/update-info", { new_name: d.name, new_phone: d.phone }); updateSessionName(d.name); toast.success("Updated"); } catch (e) { toast.error(getErrorMessage(e)); } finally { setLi(false); } }

  return (
    <>
      <Header title="My Profile" description="Manage your teacher account" />
      <PageShell>
        <div className="max-w-2xl space-y-6">
          <Card><CardContent className="pt-6">
            <div className="flex items-center gap-5">
              ...
              />
              <div><p className="font-semibold">{displaySession?.user.name ?? "Teacher"}</p><p className="text-sm text-muted-foreground">{displaySession?.user.email}</p><p className="text-xs text-primary mt-0.5 font-medium">Teacher</p></div>
            </div>
          </CardContent></Card>
          ...
          <ChangeEmailCard endpoint="/teacher/update-email" />
          <ChangePasswordCard endpoint="/teacher/update-password" />
          <SchoolInfoCard />
        </div>
      </PageShell>
    </>
