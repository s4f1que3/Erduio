...
@Injectable()
export class TeacherLogger implements NestInterceptor {
    constructor (
     ...
     async intercept(context: ExecutionContext, next: CallHandler): Promise <Observable <any> > {
        ...
        const message = this.reflector.get<string>('TeacherLogMessage', context.getHandler())
        ...
        const teacher_id = req.params.id ?? req.params.teacher_id

        const auth_id = await this.swap.swapUUIDFromIdToAuth(school_id, teacher_id)
