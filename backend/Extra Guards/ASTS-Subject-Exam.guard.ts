...
export const ASTS_Subject_Exam_Guard = () => {
    @Injectable()
    class ASTS_Subject_ExamMixin implements CanActivate {
        ...
        async canActivate(context: ExecutionContext): Promise<boolean> {
            ...
            const {data: edata, error: eerror} = await this.supabase.db
            .from('Exams')
            .select('subject_id')
            ...
            if(!edata?.subject_id) throw new NotFoundException('No subject found for assignment')
            ...
            .select('students_id')
            .eq('school_id', school_id)
            .eq('subjects_id', edata?.subject_id)
            .eq('students_id', user_id)
            .maybeSingle()

            if(serror) throw new InternalServerErrorException(serror.message)
            if(!sdata?.students_id) throw new ForbiddenException('You are not authorized to access this')

            return true
        }
    }

    return mixin(ASTS_Subject_ExamMixin)
}